...
function NormalizarRuta(str) {
  return decodeURIComponent((str || "").trim())
    .replace(/^\/+|\/+$/g, "")        // sin / al inicio/fin
    .replace(/\s+/g, " ");            // espacios normalizados
}

function RutaDesdeHref(a) {
  const href = a.getAttribute("href") || "";
  // Si el enlace viene como ?pag=... o ?page=...
  if (href.includes("?")) {
    const qs = href.split("?")[1];
    const sp = new URLSearchParams(qs);
    const r = sp.get("pag") || sp.get("page");
    if (r) return NormalizarRuta(r);
  }
  // Si viene como "Configuracion/Perfil" (sin query)
  return NormalizarRuta(href);
}

function setNavActive() {
  // 1) Ruta actual desde el query, si no existe, caemos al href “plano”
  const params = new URLSearchParams(location.search);
  const rutaActualQuery = params.get("pag") || params.get("page");
  const rutaActual = NormalizarRuta(rutaActualQuery) || NormalizarRuta(location.pathname);

  // 2) Recorrer todos los enlaces del menú y comparar su “ruta lógica”
  const items = document.querySelectorAll(".side-menu__item");
  items.forEach((a) => {
    const rutaItem = RutaDesdeHref(a);

    if (rutaItem && rutaItem === rutaActual) {
      // marcar activo
      ...
      // 3) Abrir todos los padres en cascada y mostrar sus <ul>
      let ul = a.closest("ul");
      while (ul) {
        const trigger = ul.previousElementSibling; // el <a> que abre este ul
        const liPadre = ul.parentElement;         // el <li> contenedor

        if (liPadre) {
          liPadre.classList.add("active", "open");
        }
        ...
        // subir un nivel
        ul = liPadre ? liPadre.closest("ul") : null;
      }
    }
  });
}
