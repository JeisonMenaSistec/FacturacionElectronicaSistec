...
function cargarEmpresasAsociadas() {
    //ModalCambiarEmpresa
    const modal = new bootstrap.Modal(document.getElementById('ModalCambiarEmpresa'));
    modal.show();
        ...
        if (lista) {
            ...
            if (data.status === 200) {
                ...
                if (empresas.length > 0) {
                    ...
                    lista.innerHTML = '<li class="list-group-item text-center">No hay empresas asociadas.</li>';
                }
            } else {
                lista.innerHTML = '<li class="list-group-item text-center">No hay empresas asociadas.</li>';
            }
        } else {
            console.error('Elemento con id "empresasAsociadas" no encontrado en el DOM.');
        }
    })
    .catch(error => {
        const lista = document.getElementById('empresasAsociadas');
        if (lista) {
            lista.innerHTML = '<li class="list-group-item text-danger text-center">Error al cargar empresas.</li>';
        }
...
function filtrarEmpresas() {
    ...
    if (buscador) {
        ...
        const empresas = document.querySelectorAll('#empresasAsociadas li.list-group-item');
        empresas.forEach(function(empresa) {
            // Solo filtrar elementos que tengan botón de seleccionar (empresas reales)
            const botonSeleccionar = empresa.querySelector('button[onclick*="seleccionarEmpresa"]');
            if (botonSeleccionar) {
                const nombreEmpresa = empresa.querySelector('.fw-medium');
                const textoEmpresa = nombreEmpresa ? nombreEmpresa.textContent.toLowerCase() : '';
